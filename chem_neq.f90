! chem_neq.f90 with kzz, dzz, vz, buoyancy and thermal diffusion
!> \file
!file has my best guess at a a correct diffusion module
...
LOGICAL :: l_continue !< criteria to continue or end simulation
...
CONTAINS
  ...
  IF (mype == cputerm) THEN
    ...
    END IF
    WRITE(*,logpmfmt) 'Vertical Advection',wind
    IF (wind) THEN
      WRITE(*,exppmfmt) 'vzcst [cm s-1]',vzcst*unitL/unitT, 'if = 0 read vz from input file'
    END IF
    IF (photochem) THEN
  ...
  INTEGER :: nbstep, nbstep_old, node, ndepth_max, itime, node_loc
...
!call get_phi = phi same as dndt here after model has converged
!will need to define phi in this subroutine as well as the things being passed through get_phi
  DO i = 1,nmol_neq
...
1002 FORMAT(5x,' time: ',es10.1,3x,' dn/n: ',es10.1,3x,' (dn/n)/dt: ',es10.1,3x, ' iter: ',i5)
  ...
  ! Current most accurate version of the diffusion module

  !-----------------------------------------------------------------------------
  ! Function to calculate eddy, thermal, molecular and buoyancy diffusion flux
  ! Requires
  !-----------------------------------------------------------------------------

  FUNCTION get_phi(j,i,node,nmol,inloc,n_loc)

  USE mod_chem, ONLY: molmass, dmol, therm_diff, molname, sigma_v

  USE mod_grid, ONLY: ndepth, nn, nnf, rhof, tt, ttf, pp, dr, drf, kzz, gr, vz
  ! nn and nnf are number denisties, rhof is a mass number density
  ! tt and ttf are temperatures, pp is a pressure
  ! dr and drf are thickness of grid domains
  ! kzz is the eddy diffusion constant gr is gravity
  ! -f quantities are those at grid interfaces so the boundary between 2 grid segments

  USE mod_cst, ONLY: kb, pi, amu, unitT, unitL, unitP, one_over_L3

  IMPLICIT NONE

  INTEGER,INTENT(IN) :: i,j ! counting variables for loops
  INTEGER,INTENT(IN) :: node ! node is nmol_neq*ndepth_max
  INTEGER,INTENT(IN) :: nmol ! number of molecules
  INTEGER,INTENT(IN) :: inloc(ndepth+1,nmol) ! number density array
  REAL,INTENT(IN)    :: n_loc(node)

  INTEGER  :: idepth, dif
  REAL     :: ntot_centre, kzz_centre, da, phi, diffmol, dt, dsh
  REAL     :: nnf_centre, vz_centre ! needed for vertical advection
  REAL     :: ntot(ndepth+1) ! total mass number density
  REAL     :: get_phi(ndepth)
  REAL     :: phi_moldiff, phi_kzz, phi_therm, phi_buoy, phi_adv ! phi terms for each component of phi
  DOUBLE PRECISION  :: mrn(ndepth)

  !Calculate ntot on cell faces - needed for correct (old) calculation of phi_kzz and phi_moldiff
  ...
  DO idepth = 1,ndepth
	  ...
	  !set inital phi value to 0
	  phi = 0

	  !Calculate total mass number density at cell centre
	  ntot_centre = (ntot(idepth)+ntot(idepth+1))/2

	  !Calculate total number dencity at the cell centre
	  nnf_centre = (nnf(idepth)+nnf(idepth+1))/2

	  !Calculate kzz coefficient at cell centre
	  kzz_centre = (kzz(idepth)+kzz(idepth+1))/2

	  !Calculate vz coefficient at the cell centre
	  vz_centre = (vz(idepth)+vz(idepth+1))/2

	  !Calculate diffusion coefficient at cell centre - Self diffusion
	  diffmol = (1./3.)*sqrt((3.*kb*tt(idepth))/(molmass(i)*amu))*(1./(sqrt(2.)*pi*dmol(i)**2.))*((kb*tt(idepth))/pp(idepth))

	  !Convert diffmol to cgs
	  diffmol = diffmol*unitL**2./unitT
	
	  !calculate abundance gradient
          da = ((n_loc(inloc(idepth+1,j))/ntot(idepth+1))-(n_loc(inloc(idepth,j))/ntot(idepth)))/dr(idepth)

	  !calculate temperature gradient
	  dt = (ttf(idepth+1) - ttf(idepth))/drf(idepth)

	  !calculate 1/density scale height
	  !currently unsure on the use of unitL unit change
	  !possible that gr(idepth) and tt(idepth) requires unit change but constants don't
	  dsh = (gr(idepth)*molmass(i)*amu)/(kb*tt(idepth))/unitL + ((1./tt(idepth))*dt)
	  !dsh = (gr(idepth)*molmass(i)*amu)/(kb*tt(idepth)) + ((1./tt(idepth))*dt)

	  !moldiff and kzz use the "old" ATMO methodology which is correct

          !calculate molecular diffusion flux
	  !uses da using a mass number density but mass cancels out with ntot_centre so only uses number density as required in equations
          phi_moldiff = -diffmol*da*ntot_centre
	  !could equally omit use of da and ntot_centre so do purely in number densities - unsure on effect of drf rather than dr atm
	  !phi_moldiff = (-diffmol*(((n_loc(inloc(idepth+1,j)))-(n_loc(inloc(idepth,j))))/drf(idepth)))
	  phi = phi + phi_moldiff

          !add eddy diffusion flux
	  !uses da using a mass number density but mass cancels out with ntot_centre so only uses number density as required in equations
          phi_kzz = -kzz_centre*da*ntot_centre
	  phi = phi + phi_kzz

	  !this is the same methodology as used in the code which Eric sent me
	  !according to the equations this should be correct

	  !add molecular buoyancy diffusion flux
	  !phi_buoy = (-diffmol*dsh*n_loc(inloc(idepth,j)))
	  phi_buoy = 0
    	  phi = phi + phi_buoy

	  !add thermal diffusion flux
	  !phi_therm = (-diffmol*n_loc(inloc(idepth,j))*therm_diff(i)*dt)/tt(idepth)
	  phi_therm = 0
    	  phi = phi + phi_therm

	  !Harry Baskett implementation of vertical advection
	  !add transport due to vertical advection
	  !vz at the centre of each grid multiplied by the number denisty of each species in each grid segment but need to take into account the directionality of vz
	  !DO loop goes from 1 to ndepth with 1 being the top of the atmosphere and ndepth the bottom

	  !for an updraft wind acts on the intersect between the ith and i+1th so take the mass number density of the i+1th grid segment  
	  IF (vz_centre >= 0) THEN
	    IF (idepth <  ndepth) THEN
	      phi_adv = (n_loc(inloc(idepth+1,j)))*vz_centre
	    END IF

	  !for a updraft at the bottom of the model there is no material to be "pulled" up from the grid below so phi_adv = 0
	    IF (idepth == ndepth) THEN
	      phi_adv = 0
	    END IF
	  END IF

	  !for a downdraft wind acts on the intersect between the ith and i-1th segments so take the mass number density of the i-1th grid segment
	  IF (vz_centre < 0) THEN
	    IF (idepth >  1) THEN
	      phi_adv = (n_loc(inloc(idepth-1,j)))*vz_centre
	    END IF

	  !for a downdraft at the top of the model there is no species to be "blown" down from the grid above so phi_adv = 0
	    IF (idepth == 1) THEN
	      phi_adv = 0
	    END IF
	  END IF
	  ...
	  !write each componet into a .dat file
	  IF (l_continue .eqv. .FALSE.) THEN

	    open(90, file="phi_components_atmo.dat", status="old", action="write", position="append") 

	    IF (idepth==1) THEN
	      write(90,*) "========================================================================="
	      write(90,*)             "= molecule number =", j 
	      write(90,*) "=  mol diff ", " eddy diff ", " buoy diff ", " therm diff ", " adv diff ="

	    END IF

	    write(90,*) phi_moldiff, phi_kzz, phi_buoy, phi_therm, phi_adv
...
DO i = 1, nfile
  ...
  DO
    ...
    ELSE IF (methodParam(i) == 6) THEN ! Three body
      READ(1,'(5(1x,a10) 5(1x,a10) 10(1x,e10.3))',END=1) &
      reactants(ireac,:),products(ireac,:),koParam(ireac,:),kinfParam(ireac,:)
    ELSE IF (methodParam(i) == 7) THEN ! Three body low pressure limit
      READ(1,'(5(1x,a10) 5(1x,a10) 5(1x,e10.3))',END=1) &
      reactants(ireac,:),products(ireac,:),koParam(ireac,:)
    END IF
...
REAL    :: ko_loc, kinf_loc, tloc, ceff_loc, zfc, exp_zfc, c_zfc, n_zfc, d_zfc &
           , K_loc, nloc, nref
...
DO idepth= 1,ndepth+1
  ...
  DO ireac = 1,nreac-nphoto
    ...
    !Calculate rate coefficient in low pressure limit
    ...
    IF (methodParam(modParam(ireac,1)) == 5) THEN
      ko_loc = ko_loc + &
    ...
    !troe formalism
    ELSE IF (methodParam(modParam(ireac,1)) == 3) then
      ...
      ! 3 body Lindemann
      ELSE IF (methodParam(modParam(ireac,1)) == 6) THEN
      ...
      ELSE IF (kinf_loc == 0.) THEN
        kf(idepth,ireac) = ko_loc
      ELSE
        kf(idepth,ireac) = (ko_loc/(1.0+ko_loc/kinf_loc))
      END IF

      ! 3 body low pressure limit
      ELSE IF (methodParam(modParam(ireac,1)) == 7) THEN

        kf(idepth,ireac) = ko_loc*nloc
      END IF
